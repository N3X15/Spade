...
public class Voronoi extends ModuleBase {
    ...
    public double getValue(double x, double y, double z) {
        ...
        for (int zCur = zInt - 2; zCur <= (zInt + 2); zCur++) {
            for (int yCur = yInt - 2; yCur <= (yInt + 2); yCur++) {
                for (int xCur = xInt - 2; xCur <= (xInt + 2); xCur++) {
                    ...
                    // inside of
                    // this unit cube.
                    final double xPos = xCur + NoiseGen.ValueNoise3D(xCur, yCur, zCur, seed);
                    final double yPos = yCur + NoiseGen.ValueNoise3D(xCur, yCur, zCur, seed + 1);
                    final double zPos = zCur + NoiseGen.ValueNoise3D(xCur, yCur, zCur, seed + 2);
        ...
        return value + (displacement * NoiseGen.ValueNoise3D((int) (Math.floor(xCandidate)), (int) (Math.floor(yCandidate)), (int) (Math.floor(zCandidate)), seed));// added seed
                                                                                                                                                                    // here, not in
                                                                                                                                                                    // original
                                                                                                                                                                    // but there
                                                                                                                                                                    // isn't a
    ...
    // / formations.
    public void enableDistance(final boolean enable) {
