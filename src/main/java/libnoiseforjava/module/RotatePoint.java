...
public class RotatePoint extends ModuleBase {
    ...
    public RotatePoint(final ModuleBase sourceModule) throws ExceptionInvalidParam {
        super(1);
    ...
    public double getValue(final double x, final double y, final double z) {
        ...
        final double nx = (x1Matrix * x) + (y1Matrix * y) + (z1Matrix * z);
        final double ny = (x2Matrix * x) + (y2Matrix * y) + (z2Matrix * z);
        final double nz = (x3Matrix * x) + (y3Matrix * y) + (z3Matrix * z);
        return sourceModules[0].getValue(nx, ny, nz);
    }
    
    public void setAngles(final double xAngle, final double yAngle, final double zAngle) {
