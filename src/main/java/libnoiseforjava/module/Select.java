...
public class Select extends ModuleBase {
    ...
    public Select(final ModuleBase sourceModuleOne, final ModuleBase sourceModuleTwo, final ModuleBase sourceModuleThree) throws ExceptionInvalidParam {
        super(3);
    ...
    public double getValue(final double x, final double y, final double z) {
        ...
        final double controlValue = sourceModules[2].getValue(x, y, z);
        double alpha;
        ...
        if (edgeFalloff > 0.0) {
            if (controlValue < (lowerBound - edgeFalloff))
                ...
                // end of the
                // selector threshold and within the smooth curve. Interpolate
                // between
                // the output values from the first and second source modules.
                final double lowerCurve = (lowerBound - edgeFalloff);
                final double upperCurve = (lowerBound + edgeFalloff);
                alpha = Interp.SCurve3((controlValue - lowerCurve) / (upperCurve - lowerCurve));
                return Interp.linearInterp(sourceModules[0].getValue(x, y, z), sourceModules[2].getValue(x, y, z), alpha);
            } else if (controlValue < (upperBound - edgeFalloff))
                // The output value from the control module is within the
                // selector
                // threshold; return the output value from the second source
                // module.
                ...
                // between
                // the output values from the first and second source modules.
                final double lowerCurve = (upperBound - edgeFalloff);
                final double upperCurve = (upperBound + edgeFalloff);
                alpha = Interp.SCurve3((controlValue - lowerCurve) / (upperCurve - lowerCurve));
                return Interp.linearInterp(sourceModules[1].getValue(x, y, z), sourceModules[0].getValue(x, y, z), alpha);
            } else
    ...
    // / module.
    public void setControlModule(final ModuleBase controlModule) {
