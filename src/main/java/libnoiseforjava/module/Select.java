...
public class Select extends ModuleBase {
	...
	public double getValue(double x, double y, double z) {
		...
		if (edgeFalloff > 0.0) {
			if (controlValue < (lowerBound - edgeFalloff))
				...
				alpha = Interp.SCurve3((controlValue - lowerCurve)
						/ (upperCurve - lowerCurve));
				return Interp.linearInterp(sourceModules[0].getValue(x, y, z),
						sourceModules[2].getValue(x, y, z), alpha);
				...
				return Interp.linearInterp(sourceModules[1].getValue(x, y, z),
						sourceModules[0].getValue(x, y, z), alpha);
			} else
	...
	public void setEdgeFalloff(double edgeFalloff) {
		...
		edgeFalloff = (edgeFalloff > boundSize / 2) ? boundSize / 2
				: edgeFalloff;
	}
