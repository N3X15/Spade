...
public class NoiseMapBuilderCylinder extends NoiseMapBuilder {
    ...
    public void build() throws ExceptionInvalidParam {
        if ((upperAngleBound <= lowerAngleBound) || (upperHeightBound <= lowerHeightBound) || (destWidth <= 0) || (destHeight <= 0) || (sourceModule == null) || (destNoiseMap == null))
        ...
        final Cylinder cylinderModel = new Cylinder();
        cylinderModel.setModule(sourceModule);
        
        final double angleExtent = upperAngleBound - lowerAngleBound;
        final double heightExtent = upperHeightBound - lowerHeightBound;
        final double xDelta = angleExtent / destWidth;
        final double yDelta = heightExtent / destHeight;
        ...
        for (int y = 0; y < destHeight; y++) {
            ...
            for (int x = 0; x < destWidth; x++) {
                final float curValue = (float) cylinderModel.getValue(curAngle, curHeight);
    ...
    // /
    // / One unit is equal to the radius of the cylinder.
    public void setBounds(final double lowerAngleBound, final double upperAngleBound, final double lowerHeightBound, final double upperHeightBound) throws ExceptionInvalidParam {
    ...
    public void setLowerAngleBound(final double lowerAngleBound) {
        this.lowerAngleBound = lowerAngleBound;
    }
    
    public void setLowerHeightBound(final double lowerHeightBound) {
        this.lowerHeightBound = lowerHeightBound;
    }
    
    public void setUpperAngleBound(final double upperAngleBound) {
        this.upperAngleBound = upperAngleBound;
    }
    
    public void setUpperHeightBound(final double upperHeightBound) {
        this.upperHeightBound = upperHeightBound;
    }
