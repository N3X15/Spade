...
public class NoiseMapBuilderSphere extends NoiseMapBuilder {
    ...
    public void build() throws ExceptionInvalidParam {
        if ((eastLonBound <= westLonBound) || (northLatBound <= southLatBound) || (destWidth <= 0) || (destHeight <= 0) || (sourceModule == null) || (destNoiseMap == null))
        ...
        final Sphere sphereModel = new Sphere();
        sphereModel.setModule(sourceModule);
        
        final double lonExtent = eastLonBound - westLonBound;
        final double latExtent = northLatBound - southLatBound;
        final double xDelta = lonExtent / destWidth;
        final double yDelta = latExtent / destHeight;
        ...
        for (int y = 0; y < destHeight; y++) {
            ...
            for (int x = 0; x < destWidth; x++) {
                final float curValue = (float) sphereModel.getValue(curLat, curLon);
    ...
    public void setBounds(final double southLatBound, final double northLatBound, final double westLonBound, final double eastLonBound) throws ExceptionInvalidParam {
    ...
    public void setEastLonBound(final double eastLonBound) {
        this.eastLonBound = eastLonBound;
    }
    
    public void setNorthLatBound(final double northLatBound) {
        this.northLatBound = northLatBound;
    }
    
    public void setSouthLatBound(final double southLatBound) {
        this.southLatBound = southLatBound;
    }
    
    public void setWestLonBound(final double westLonBound) {
        this.westLonBound = westLonBound;
    }
