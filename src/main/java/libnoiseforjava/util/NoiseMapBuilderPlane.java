...
public class NoiseMapBuilderPlane extends NoiseMapBuilder {
	...
	public NoiseMapBuilderPlane(int height, int width)
	...
	public void build() throws ExceptionInvalidParam {
		if (upperXBound <= lowerXBound || upperZBound <= lowerZBound
				|| destWidth <= 0 || destHeight <= 0 || sourceModule == null
				|| destNoiseMap == null)
			throw new ExceptionInvalidParam(
					"Invalid parameter in NoiseMapBuilderPlane");
		...
		for (int z = 0; z < destHeight; z++) {
			...
			for (int x = 0; x < destWidth; x++) {
				...
				if (!isSeamlessEnabled)
					...
					neValue = planeModel.getValue(xCur + xExtent, zCur
							+ zExtent);
