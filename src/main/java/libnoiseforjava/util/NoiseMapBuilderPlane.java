...
public class NoiseMapBuilderPlane extends NoiseMapBuilder {
    ...
    public NoiseMapBuilderPlane(final int height, final int width) throws ExceptionInvalidParam {
        super(height, width);
        isSeamlessEnabled = false;
        lowerXBound = 0.0;
        lowerZBound = 0.0;
    ...
    public void build() throws ExceptionInvalidParam {
        if ((upperXBound <= lowerXBound) || (upperZBound <= lowerZBound) || (destWidth <= 0) || (destHeight <= 0) || (sourceModule == null) || (destNoiseMap == null))
        ...
        final Plane planeModel = new Plane();
        planeModel.setModule(sourceModule);
        
        final double xExtent = upperXBound - lowerXBound;
        final double zExtent = upperZBound - lowerZBound;
        final double xDelta = xExtent / destWidth;
        final double zDelta = zExtent / destHeight;
        ...
        for (int z = 0; z < destHeight; z++) {
            ...
            for (int x = 0; x < destWidth; x++) {
                ...
                if (!isSeamlessEnabled) {
                    ...
                    final double xBlend = 1.0 - ((xCur - lowerXBound) / xExtent);
                    final double zBlend = 1.0 - ((zCur - lowerZBound) / zExtent);
                    final double z0 = Interp.linearInterp(swValue, seValue, xBlend);
                    final double z1 = Interp.linearInterp(nwValue, neValue, xBlend);
    ...
    public void enableSeamless(final boolean enable) {
    ...
    public void setBounds(final double lowerXBound, final double upperXBound, final double lowerZBound, final double upperZBound) throws ExceptionInvalidParam {
    ...
    public void setLowerXBound(final double lowerXBound) {
        this.lowerXBound = lowerXBound;
    }
    
    public void setLowerZBound(final double lowerZBound) {
        this.lowerZBound = lowerZBound;
    }
    
    public void setUpperXBound(final double upperXBound) {
        this.upperXBound = upperXBound;
    }
    
    public void setUpperZBound(final double upperZBound) {
        this.upperZBound = upperZBound;
    }
