...
public class RendererImage {
	...
	public void addGradientPoint(double gradientPos, ColorCafe gradientColor)
	...
	public ColorCafe calcDestColor(ColorCafe sourceColor,
			ColorCafe backgroundColor, double lightValue) {
		...
		double green = Interp.linearInterp(backgroundGreen, sourceGreen,
				sourceAlpha);
		double blue = Interp.linearInterp(backgroundBlue, sourceBlue,
				sourceAlpha);
		...
		// return
		// the new color.
		ColorCafe newColor = new ColorCafe((int) (red * 255.0) & 0xff,
				(int) (green * 255.0) & 0xff, (int) (blue * 255.0) & 0xff,
	...
	public double calcLightIntensity(double center, double left, double right,
	...
	public void render() throws ExceptionInvalidParam {
		...
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				...
				// noise
				// map.
				ColorCafe destColor = gradient.getColor(sourceNoiseMap
						.getValue(x, y));
				...
				if (isLightEnabled) {
					...
					double nl = (sourceNoiseMap.getValue(x
							+ xLeftOffset, y));
					double nr = (sourceNoiseMap.getValue(x
							+ xRightOffset, y));
					double nd = (sourceNoiseMap.getValue(x, y
							+ yDownOffset));
					double nu = (sourceNoiseMap.getValue(x, y
							+ yUpOffset));
