...
public class RendererImage {
    ...
    public void addGradientPoint(final double gradientPos, final ColorCafe gradientColor) throws ExceptionInvalidParam {
    ...
    public ColorCafe calcDestColor(final ColorCafe sourceColor, final ColorCafe backgroundColor, final double lightValue) {
        final double sourceRed = sourceColor.red / 255.0;
        final double sourceGreen = sourceColor.green / 255.0;
        final double sourceBlue = sourceColor.blue / 255.0;
        final double sourceAlpha = sourceColor.alpha / 255.0;
        final double backgroundRed = backgroundColor.red / 255.0;
        final double backgroundGreen = backgroundColor.green / 255.0;
        final double backgroundBlue = backgroundColor.blue / 255.0;
        ...
        if (isLightEnabled) {
            ...
            final double lightRed = (lightValue * lightColor.red) / 255.0;
            final double lightGreen = (lightValue * lightColor.green) / 255.0;
            final double lightBlue = (lightValue * lightColor.blue) / 255.0;
        ...
        // return
        // the new color.
        final ColorCafe newColor = new ColorCafe((int) (red * 255.0) & 0xff, (int) (green * 255.0) & 0xff, (int) (blue * 255.0) & 0xff, Math.max(sourceColor.alpha, backgroundColor.alpha));
        return newColor;
    }
    ...
    public double calcLightIntensity(final double center, final double left, final double right, final double down, final double up) {
        ...
        final double I_MAX = 1.0;
        final double io = (I_MAX * SQRT_2 * sinElev) / 2.0;
        final double ix = (I_MAX - io) * lightContrast * SQRT_2 * cosElev * cosAzimuth;
        final double iy = (I_MAX - io) * lightContrast * SQRT_2 * cosElev * sinAzimuth;
    ...
    public void render() throws ExceptionInvalidParam {
        if ((sourceNoiseMap == null) || (destImageCafe == null) || (sourceNoiseMap.getWidth() <= 0) || (sourceNoiseMap.getHeight() <= 0) || (gradient.getGradientPointCount() < 2))
            throw new ExceptionInvalidParam("Invalid Parameter in RendererImage");
        
        final int width = sourceNoiseMap.getWidth();
        final int height = sourceNoiseMap.getHeight();
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                // noise
                // map.
                final ColorCafe destColor = gradient.getColor(sourceNoiseMap.getValue(x, y));
                ...
                if (isLightEnabled) {
                    ...
                    final double nc = (sourceNoiseMap.getValue(x, y));
                    final double nl = (sourceNoiseMap.getValue(x + xLeftOffset, y));
                    final double nr = (sourceNoiseMap.getValue(x + xRightOffset, y));
                    final double nd = (sourceNoiseMap.getValue(x, y + yDownOffset));
                    final double nu = (sourceNoiseMap.getValue(x, y + yUpOffset));
    ...
    // / tileable textures.
    public void enableWrap(final boolean enable) {
    ...
    public void setBackgroundImage(final ImageCafe backgroundImage) {
    ...
    // / The destination image must exist throughout the lifetime of this
    // / object unless another image replaces that image.
    public void setDestImage(final ImageCafe destImage) {
        destImageCafe = destImage;
    }
    ...
    public void setLightAzimuth(final double lightAzimuth) {
    ...
    // / Make sure the light source is enabled via a call to the
    // / EnableLight() method before calling the Render() method.
    public void setLightBrightness(final double lightBrightness) {
    ...
    // / Make sure the light source is enabled via a call to the
    // / EnableLight() method before calling the Render() method.
    public void setLightColor(final ColorCafe lightColor) {
    ...
    // / Make sure the light source is enabled via a call to the
    // / EnableLight() method before calling the Render() method.
    public void setLightContrast(final double lightContrast) throws ExceptionInvalidParam {
    ...
    // / Make sure the light source is enabled via a call to the
    // / EnableLight() method before calling the Render() method.
    public void setLightElev(final double lightElev) {
    ...
    // / Make sure the light source is enabled via a call to the
    // / enableLight() method before calling the render() method.
    public void setLightIntensity(final double lightIntensity) throws ExceptionInvalidParam {
    ...
    // / The destination image must exist throughout the lifetime of this
    // / object unless another image replaces that image.
    public void setSourceNoiseMap(final NoiseMap sourceNoiseMap) {
