...
public class RendererNormalMap {
    ...
    public RendererNormalMap(final int height, final int width) throws ExceptionInvalidParam {
        bumpHeight = 1.0;
        isWrapEnabled = false;
    ...
    // / the application.
    public ColorCafe calcNormalColor(double nc, double nr, double nu, final double bumpHeight) {
        // Calculate the surface normal.
        nc *= bumpHeight;
        nr *= bumpHeight;
        nu *= bumpHeight;
        final double ncr = (nc - nr);
        final double ncu = (nc - nu);
        final double d = Math.sqrt((ncu * ncu) + (ncr * ncr) + 1);
        final double vxc = (nc - nr) / d;
        final double vyc = (nc - nu) / d;
        final double vzc = 1.0 / d;
    ...
    public void render() throws ExceptionInvalidParam {
        if ((sourceNoiseMap == null) || (destImageCafe == null) || (sourceNoiseMap.getWidth() <= 0) || (sourceNoiseMap.getHeight() <= 0))
    ...
    // / The spatial resolution and elevation resolution are determined by
    // / the application.
    public void setBumpHeight(final double bumpHeight) {
    ...
    public void setDestImageCafe(final ImageCafe destImageCafe) {
