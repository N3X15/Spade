...
public class RendererNormalMap {
	...
	public RendererNormalMap(int height, int width)
	...
	public void render() throws ExceptionInvalidParam {
		if (sourceNoiseMap == null || destImageCafe == null
			...
			throw new ExceptionInvalidParam(
					"Invalid Parameter in RendererNormalMap");
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				double nr = (sourceNoiseMap.getValue(
						(x + xRightOffset), y));
				double nu = (sourceNoiseMap.getValue(x,
						(y + yUpOffset)));

				// Calculate the normal product.
				destImageCafe.setValue(x, y,
						(calcNormalColor(nc, nr, nu, bumpHeight)));
