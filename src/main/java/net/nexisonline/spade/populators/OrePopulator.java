...
public class OrePopulator extends SpadeEffectGenerator {
    ...
    public class DepositDef {
         ...
         * @param maxDeposits
         */
        public DepositDef(final DepositType depositType, final int blockType, final int minHeight, final int maxHeight, final int minBlocks, final int maxBlocks, final int minDeposits, final int maxDeposits) {
         ...
         */
        public DepositDef(final DepositType depositType, final int blockType, final int minHeight, final int maxHeight, final int rarity) {
            this.depositType = depositType;
            this.blockType = blockType;
            this.minHeight = minHeight;
            this.maxHeight = maxHeight;
        ...
        public void populate(final Chunk chunk, final int X, final int Z, final Random random) {
            this.chunk = chunk;
            x_o = X * 16;
            z_o = Z * 16;
            this.random = random;
        ...
        private void addManyTimes() {
            ...
            final int rarityshift = (rare) ? 8 : 0;
            for (int i = 0; i < max; i++) {
                final int x = x_o + random.nextInt(16) + rarityshift;
                final int y = randInRange(minHeight, Math.min(127, maxHeight));
                final int z = z_o + random.nextInt(16) + rarityshift;
                addDeposit(x, y, z);
            }
        }
        
        private int randInRange(final int min, final int max) {
        ...
        private void addDeposit(final int x, final int y, final int z) {
            try {
                switch (depositType) {
                    case LAKE:
        ...
        private net.minecraft.server.World getMCWorld() {
            final net.minecraft.server.World w = ((CraftWorld) chunk.getWorld()).getHandle();
            if (w == null) {
    ...
    private final List<DepositDef> oreDefs = new ArrayList<DepositDef>();
    private final Random random;
    private List<Object> deposits;
    private final List<ChunkCoordinates> postponedChunks = new ArrayList<ChunkCoordinates>();
    
    @SuppressWarnings("unchecked")
    public OrePopulator(final SpadePlugin plugin, final Map<String, Object> node, final long seed) {
        ...
        for (final Object deposit_o : deposits) {
            final Map<String, Object> deposit = (Map<String, Object>) deposit_o;
            final DepositDef od = new DepositDef();
            od.depositType = DepositType.valueOf(SpadeConf.getString(deposit, "depositType", "blob"));
            final Object bt = deposit.get("blockType");
    ...
    public void onWorldLoaded(final World w) {
        final Random rnd = new Random(w.getSeed());
        world = w;
        loaded = true;
        for (final ChunkCoordinates cc : postponedChunks) {
            if (cc == null) {
                continue;
            }
            final Chunk c = world.getChunkAt(cc.x, cc.z);
            populate(w, rnd, c);
        }
    }
    ...
    public void populate(final World world, final Random rand, final Chunk chunk) {
        ...
        final long xseed = ((random.nextLong() / 2L) * 2L) + 1L;
        final long zseed = ((random.nextLong() / 2L) * 2L) + 1L;
        random.setSeed(((chunk.getX() * xseed) + (chunk.getZ() * zseed)) ^ world.getSeed());
        // END Notchcode
        
        for (final DepositDef ore : oreDefs) {
    ...
    public Map<String, Object> getConfiguration() {
        ...
        final List<Object> deposits = new ArrayList<Object>();
        for (final DepositDef def : oreDefs) {
