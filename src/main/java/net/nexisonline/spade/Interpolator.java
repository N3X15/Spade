...
public class Interpolator {
     ...
     */
    private static int dmi(final int x, final int y, final int z) {
        return (x << 11) | (z << 7) | y;
    }
     ...
     */
    public static double[] LinearExpand(final double[] abyte) {
        // Generate the xy and yz planes of blocks by interpolation
        /*
         * for (int x = 0; x < 16; x += 3) { for (int y = 0; y < 128; y += 3) { for (int z = 0; z < 16; z += 3) { if (y != 15) { abyte[arrayindex(x , y + 1, z)] = lerp(abyte[arrayindex(x, y, z)], abyte[arrayindex(x, y + 3, z)], 0.02f); abyte[arrayindex(x, y + 2, z)] = lerp(abyte[arrayindex(x, y, z)], abyte[arrayindex(x, y + 3, z)], 0.98f); } } } }
         */
     ...
     * @return
     */
    public static Heightmap LinearExpandHeightmap(final Heightmap hm, final int newheight, final int newwidth) {
        double x_coord = 0;
        double z_coord = 0;
        double val1 = 0;
        double val2 = 0;
        final Heightmap newhm = new Heightmap(newheight, newwidth);
        final double height_ratio = (double) hm.height / newheight;
        final double width_ratio = (double) hm.width / newwidth;
        for (int y = 0; y < newheight; y++) {
            z_coord = (y * height_ratio) - 0.5f;
            final int y_coord_lo = (int) z_coord;
            final int y_coord_hi = y_coord_lo + 1;
            for (int x = 0; x < newwidth; x++) {
                x_coord = (x * width_ratio) - 0.5f;
                final int x_coord_lo = (int) x_coord;
                final int x_coord_hi = x_coord_lo + 1;
                final float x_diff = (float) (x_coord - x_coord_lo);
     ...
     * @return
     */
    public static Densitymap _DERPLinearExpandDensitymap(final Densitymap hm, final int newheight, final int newdepth, final int newwidth) {
        final Densitymap ndm = new Densitymap(newheight, newdepth, newwidth);
        double x_coord = 0;
        double y_coord = 0;
        double z_coord = 0;
        double nl = 0;
        ...
        final double height_ratio = (double) hm.height / newheight;
        final double width_ratio = (double) hm.width / newwidth;
        final double depth_ratio = (double) hm.depth / newdepth;
        for (int z = 0; z < newheight; z++) {
            z_coord = (z * height_ratio) - 0.5f;
            final int z_coord_lo = (int) z_coord;
            final int z_coord_hi = z_coord_lo + 1;
            for (int x = 0; x < newwidth; x++) {
                x_coord = (x * width_ratio) - 0.5f;
                for (int y = 0; y < newdepth; y++) {
                    ...
                    final int x_coord_hi = x_coord_lo + 1;
                    final float x_diff = (float) (x_coord - x_coord_lo);
                    final float z_diff = (float) (z_coord - z_coord_lo);
                    final int y_coord_lo = (int) y_coord;
                    final int y_coord_hi = y_coord_lo + 1;
                    final float y_diff = (float) (y_coord - y_coord_lo);
                    // north, lower
    ...
    public static Densitymap LinearExpandDensitymap(final Densitymap hm, final int newheight, final int newdepth, final int newwidth) {
        ...
        for (int z = 0; z < newheight; z++) {
            for (int x = 0; x < newdepth; x++) {
                for (int y = 0; y < newwidth; y++) {
                    final float i = (float) ((((float) x) * 1) / 0.25);
                    final float j = (float) (((y) / 0.25) * 1);
                    final float k = (float) ((((float) z) * 1) / 0.25);
     ...
     */
    public static Heightmap OLD_LinearExpandHeightmap(final Heightmap hm) {
        // Generate the xz plane of blocks by interpolation
        for (int x = 0; x < 16; x += 3) {
            for (int z = 0; z < 16; z += 3) {
                if ((x == 0) && (z > 0)) {
    ...
    private static double lerp(final double a, final double b, final float f) {
        return ((a * (1 - f)) + (b * f));
    }
}
